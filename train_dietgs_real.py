...
def training(
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration > -1 and (iteration-1) % 162 == 0: 
        ...
        img_remap= F.interpolate(image.unsqueeze(0), scale_factor=4, mode='bicubic', align_corners=False)
        img_remap = (img_remap * 2.0) - 1.0
        latent = guidance.encode_images(img_remap).movedim(1, -1)

        latent, org_image, mask = sample_patch(
            latent=latent,
