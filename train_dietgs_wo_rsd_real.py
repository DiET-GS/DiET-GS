...
def training(
    ...
    if event:
        ...
        with open(os.path.join(dataset.source_path, "events/id_to_color.pickle"), "rb") as f:
            id_to_color_map = torch.from_numpy(pickle.load(f)).to(device)
        ...
        interval = (all_tms[1] - all_tms[0])

        all_timestamps = []
        for tms_start, tms_end in zip(img_start_stamp, img_end_stamp):
            all_timestamps.append(np.linspace(tms_start, tms_end, 9))
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if event:
            ...
            event_start_stamp = random.randint(events[:, 1][0], events[:, 1][-1] - interval)
            ...
            event_start_pose = interpolate_poses(torch.tensor([event_start_stamp]), all_timestamps.cpu(), all_poses.cpu())
            event_end_pose = interpolate_poses(torch.tensor([event_start_stamp + interval]), all_timestamps.cpu(), all_poses.cpu())
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), event, metric)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, event, metric):
    if tb_writer:
